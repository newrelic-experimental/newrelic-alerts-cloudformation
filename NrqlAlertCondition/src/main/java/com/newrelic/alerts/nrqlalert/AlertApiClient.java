...
public class AlertApiClient {
    ...
    public JSONObject create(NewRelicNrqlCondition newRelicNrqlCondition, String apiKey, int policyId) throws IOException, AlertApiException {
        ...
        alertPost.addHeader("ACCEPT", "application/json");
        ...
        if (statusCode >= 400) {
            String message = new BufferedReader(new InputStreamReader(response.getEntity().getContent(), StandardCharsets.UTF_8)).lines().collect(Collectors.joining("\n"));
            // Consume entity to avoid leaking the connection
            EntityUtils.consume(response.getEntity());
            throw new AlertApiException(String.format("Error creating alert condition. Status code: %d Policy ID: %d Error message: '%s'", statusCode, policyId, message));
        } else {
    ...
    public List<NewRelicNrqlCondition> list(String apiKey, int policyId, @Nullable Integer page) throws IOException, AlertApiException {
        ...
        if (statusCode >= 400) {
            ...
            if (statusCode == 401) {
                throw new AlertApiException(String.format("Unauthorized. Failed to list conditions for policy %d. Probably an invalid API key.", policyId));
            } else {
                throw new AlertApiException(String.format("Failed to list alert conditions for policy: %d", policyId));
            }
        }

        Map<String, List<NewRelicNrqlCondition>> response = mapper.readValue(httpResponse.getEntity().getContent(), new TypeReference<Map<String, List<NewRelicNrqlCondition>>>() {
        });
