...
public class CreateHandler extends BaseHandler<CallbackContext> {
    ...
    @Override
    public ProgressEvent<ResourceModel, CallbackContext> handleRequest(final AmazonWebServicesClientProxy proxy, final ResourceHandlerRequest<ResourceModel> request, final CallbackContext callbackContext, final Logger logger) {
        ...
        try {
            logger.log(String.format("Attempting to create alert condition for policy ID: %d  condition: %s", policyId, newRelicNrqlCondition));
